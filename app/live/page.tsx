        ...
        <PageContainer className={styles.content}><section className={styles.creatorsSection}>
            ...
            <div className={styles.creatorGrid}>
                {creators.map((creator) => <Link key={creator.twitchLogin} href={creator.channelUrl} target="_blank" rel="noreferrer" className={styles.creatorCard}>
                    ...
                    <div className={styles.creatorBody}><div className={styles.avatar}>{creator.initials}</div><div><strong>{creator.displayName}</strong><p>{creator.description}</p><span>twitch.tv/{creator.twitchLogin}</span></div><b aria-hidden="true">→</b></div>
                </Link>)}
            </div>
