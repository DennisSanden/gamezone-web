---
title: "By"
description: "Den tredje settlementnivån där handel och företag blir en naturlig del av settlementets utveckling."
category: "Settlements"
order: 4
version: "1.0"
engineVersion: "Settlement Registry"
updatedAt: "2026-07-18"
infoboxTitle: "By"
infobox:
  nivå: "3"
  territorieradie: "60 block"
  veckounderhåll: "1 000 Coins"
  grundbonus: "+15 % Coins"
  handelsrabatt: "10 %"
  nästaNivå: "Bosättning"
relatedArticles:
  - category: "settlements"
    article: "lager"
    title: "Läger"
    description: "Den andra settlementnivån och föregående steg i settlementets utveckling."
  - category: "settlements"
    article: "bosattning"
    title: "Bosättning"
    description: "Den fjärde settlementnivån och nästa steg i settlementets utveckling."
---

<SettlementInfoBox settlement="by" />

## Översikt

**By** är den tredje av GameZones femton settlementnivåer.

På denna nivå blir settlementet ett etablerat samhälle med organiserad handel. Handelsrabatten ökar ytterligare och den första samhällsbyggnaden blir tillgänglig genom Handelscentrum, vilket också låser upp möjligheten att registrera företag.

> [!INFO] Settlementnivån avgör territoriets storlek, handelsrabatten, veckounderhållet och vilka byggnader och funktioner som är tillgängliga.

## Territorium

Ett settlement på nivån By kontrollerar ett cirkulärt territorium med en radie på **60 block** från settlementets registrerade centrum.

Territoriet utökas automatiskt när settlementet når nivå 3.

Settlementets registrerade centrum är permanent och kan inte flyttas.

Det finns ingen fysisk Town Hall som kan förstöras, flyttas eller användas som ett vanligt Minecraft-block.

## Grundbonus

Alla aktiva invånare får settlementets permanenta grundbonus på **+15 % Coins** från all reward-eligible produktion.

Grundbonusen gäller:

- Gruvdrift
- Jordbruk
- Skogsbruk
- Boskap
- Fiske
- Byggmaterial
- Alkemi

Bonusar från upplåsta byggnader läggs ovanpå grundbonusen.

> [!IMPORTANT] Grundbonusen gäller endast settlementets aktiva medlemmar och endast manuell produktion som är reward-eligible.

## Handelsrabatt

Företag som är registrerade i ett settlement på nivån By ger alla spelare en handelsrabatt på **10 %**.

Rabatten påverkar endast köparens slutpris.

Företaget får fortfarande hela sitt satta försäljningspris och mellanskillnaden finansieras av Economy Engine.

Settlementets ekonomi påverkas inte av handelsrabatten. Rabatten finansieras helt av Economy Engine.

## Veckounderhåll

By har ett veckounderhåll på **1 000 Coins**.

By har ett veckounderhåll på **1 000 Coins**.

Det veckovisa underhållet måste betalas för att settlementet ska behålla sin nivå.

Om underhållet inte kan betalas riskerar settlementet att nedgraderas. Permanenta byggnadsupplåsningar försvinner inte vid en nedgradering, men byggnader vars nivåkrav inte längre uppfylls blir inaktiva.

När settlementet åter når rätt nivå aktiveras byggnaderna automatiskt igen.

Om stadskassan saknar tillräckligt med Coins kan settlementet nedgraderas. Permanenta byggnadsupplåsningar försvinner inte vid en nedgradering, men byggnader vars nivåkrav inte längre uppfylls blir inaktiva.

När settlementet åter når rätt nivå aktiveras byggnaderna igen utan att behöva betalas en andra gång.

## Byggnader

<SettlementBuildingsPanel group="by" />

## Nästa nivå

<SettlementUpgradePanel upgradeKey="by-till-bosattning" />
